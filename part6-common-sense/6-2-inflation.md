...
keywords: ["인플레이션", "물가", "실질수익률", "구매력", "물가상승률", "현금 가치", "명목"]
part: 6
order: 2
date: 2026-08-10
---
...
우리가 "물가가 몇 % 올랐다"고 할 때 그 근거가 되는 게 **소비자물가지수**예요.
...
물가가 주식에 좋은지 나쁜지는 한쪽으로 못 박을 수 없어요. 매출과 자산 가치가 함께 오르는 면도 있고, 원가가 올라 이익이 깎이는 면도 있거든요. **다만 시장을 실제로 크게 흔드는 건 세 번째 경로예요 — 물가가 오르면 금리를 올려 잡으려 하고, 그때부터 [6-1](6-1-interest-rate-and-stocks.md)의 세 경로가 작동해요.**
