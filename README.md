# 주식·투자 상식 사전 — 계좌 개설부터 ETF·연금까지

주식이 처음인 분들을 위한 투자 상식 사전이에요. 증권계좌를 아직 안 만들어본 완전 초보부터, 매매는 해봤지만 개념 정리가 안 된 입문자까지를 대상으로 해요. 각 챕터는 검색으로 궁금할 만한 질문 하나에 완결로 답하는 독립적인 글이라서, 처음부터 순서대로 읽어도 되고 지금 궁금한 챕터만 골라 봐도 괜찮아요.

읽는 순서를 추천하자면 파트 1부터 6까지 차례로 읽는 걸 권해요. 뒤 챕터는 앞 챕터 지식을 살짝 전제로 하는 경우가 있는데, 그럴 땐 본문에 한두 줄 요약과 함께 관련 챕터 링크를 달아둘 거예요. 그래서 순서를 안 지키고 봐도 흐름을 놓치지 않아요.

파트 7(투자 트렌드)은 다른 파트와 성격이 달라요. 미리 다 써놓는 챕터 목록이 아니라, 그때그때 유행하는 투자 방식이나 이슈가 생길 때마다 계속 새 글이 추가되는 수시 업데이트 파트예요. 지금은 7-1과 7-2 두 편이 있고, 앞으로 계속 늘어날 예정이에요.

## 전체 목차

### 파트 1. 주식 투자, 처음이라면 (완전 초보)

- [x] [1-1. 주식이란 무엇인가 — 회사 지분을 산다는 것의 의미](part1-basics/1-1-what-is-stock.md)
- [x] [1-2. 증권계좌 개설, 어디서 어떻게 하나](part1-basics/1-2-open-brokerage-account.md)
- [x] [1-3. 주식 사는 법 — 호가창, 시장가/지정가 주문 이해하기](part1-basics/1-3-how-to-buy-stocks.md)
- [x] [1-4. 매매 시간과 장전·장후 거래](part1-basics/1-4-trading-hours.md)
- [x] [1-5. 수수료와 세금, 주식 사고팔면 뭘 떼일까](part1-basics/1-5-fees-and-taxes.md)
- [x] [1-6. 시가총액·주가·거래량, 숫자 읽는 법](part1-basics/1-6-market-cap-price-volume.md)

### 파트 2. 국내 주식시장 이해하기

- [x] [2-1. 코스피 vs 코스닥, 뭐가 다른가](part2-korea-market/2-1-kospi-vs-kosdaq.md)
- [x] [2-2. 코스피 지수는 어떻게 계산되나](part2-korea-market/2-2-how-kospi-index-works.md)
- [x] [2-3. 배당이란 — 배당락, 배당 기준일, 배당수익률](part2-korea-market/2-3-dividend-basics.md)
- [x] [2-4. 공모주 청약, 어떻게 참여하나](part2-korea-market/2-4-ipo-subscription.md)
- [x] [2-5. 유상증자·무상증자·액면분할이 주가에 미치는 영향](part2-korea-market/2-5-capital-increase-and-split.md)
- [x] [2-6. 상한가·하한가·서킷브레이커 — 시장의 안전장치](part2-korea-market/2-6-price-limit-circuit-breaker.md)
- [x] [2-7. 공매도란 무엇이고 왜 논란인가](part2-korea-market/2-7-short-selling.md)

### 파트 3. 미국주식·해외투자

- [x] [3-1. 미국주식, 왜 하는가 — 국내주식과의 차이](part3-us-overseas/3-1-why-us-stocks.md)
- [x] [3-2. 미국주식 거래 시간과 프리마켓·애프터마켓](part3-us-overseas/3-2-us-trading-hours.md)
- [x] [3-3. 미국주식 세금 — 양도소득세 250만원 공제 이해하기](part3-us-overseas/3-3-us-stock-tax.md)
- [x] [3-4. 환율과 환전 — 환헤지 vs 환노출](part3-us-overseas/3-4-exchange-rate.md)
- [x] [3-5. S&P500·나스닥·다우 — 3대 지수 구분하기](part3-us-overseas/3-5-us-indices.md)
- [x] [3-6. 미국 배당주와 배당 성장 투자](part3-us-overseas/3-6-us-dividend-investing.md)

### 파트 4. ETF·펀드·연금으로 하는 투자

- [x] [4-1. ETF란 무엇인가 — 펀드와 뭐가 다른가](part4-etf-pension/4-1-what-is-etf.md)
- [x] [4-2. 인덱스 투자 — 왜 "시장을 사라"고 하는가](part4-etf-pension/4-2-index-investing.md)
- [x] [4-3. ETF 고르는 법 — 운용보수, 괴리율, 추적오차](part4-etf-pension/4-3-how-to-choose-etf.md)
- [x] [4-4. 연금저축펀드 vs IRP — 세액공제 활용법](part4-etf-pension/4-4-pension-fund-vs-irp.md)
- [x] [4-5. ISA 계좌, 왜 만능통장이라 부르나](part4-etf-pension/4-5-isa-account.md)
- [x] [4-6. 절세 계좌 3종 우선순위 — 뭐부터 채워야 하나](part4-etf-pension/4-6-tax-account-priority.md)

### 파트 5. 기업분석·재무제표 입문

- [x] [5-1. 재무제표, 어디서 보고 뭘 봐야 하나 (DART 사용법)](part5-analysis/5-1-how-to-read-dart.md)
- [x] [5-2. PER — 이 주식은 비싼가 싼가](part5-analysis/5-2-per.md)
- [x] [5-3. PBR과 ROE — 자산과 수익성으로 보는 기업](part5-analysis/5-3-pbr-roe.md)
- [x] [5-4. 매출·영업이익·순이익, 손익계산서 읽기](part5-analysis/5-4-income-statement.md)
- [x] [5-5. 부채비율과 유동비율 — 망하지 않을 회사 고르기](part5-analysis/5-5-debt-ratio.md)
- [x] [5-6. 가치평가의 한계 — 지표만 보면 안 되는 이유](part5-analysis/5-6-limits-of-valuation.md)

### 파트 6. 알아두면 돈이 되는 투자 상식

- [x] [6-1. 금리와 주가의 관계 — 왜 금리가 오르면 주가가 떨어지나](part6-common-sense/6-1-interest-rate-and-stocks.md)
- [x] [6-2. 인플레이션과 투자 — 현금은 왜 녹는가](part6-common-sense/6-2-inflation.md)
- [x] [6-3. 분산투자와 포트폴리오 — 계란을 나눠 담는 법](part6-common-sense/6-3-diversification.md)
- [x] [6-4. 투자 심리 — 물타기, 손절, FOMO를 다루는 법](part6-common-sense/6-4-investor-psychology.md)
- [x] [6-5. 복리의 힘 — 장기투자가 유리한 수학적 이유](part6-common-sense/6-5-compound-interest.md)
- [x] [6-6. 경제 뉴스 읽는 법 — FOMC, CPI, 고용지표가 뭐길래](part6-common-sense/6-6-reading-economic-news.md)
- [x] [6-7. 흔한 투자 사기 유형과 피하는 법](part6-common-sense/6-7-investment-scams.md)

### 파트 7. 투자 트렌드 (수시 업데이트)

이 파트는 다른 파트와 달리 목차가 고정되어 있지 않아요. 그때그때 유행하는 투자 방식이나 시장 이슈가 생길 때마다 독립된 글로 계속 추가돼요. 낡은 글이라도 지우지 않고 아카이브로 남겨둘 예정이에요.

- [x] [7-1. 2026년 하반기 시장 지형 — 2026년 8월 시점에서 본 것들](part7-trends/2026-h2-market-trends.md)
- [x] [7-2. 조각투자와 토큰증권, 제도는 어디까지 왔나 — 2026년 8월 시점의 기록](part7-trends/2026-fractional-investment-sto.md)

## 집필 순서

파트 1 → 2 → 3 → 4 → 5 → 6 순서로 순차 집필해요. 초보 유입 검색량이 많고 뒤 챕터의 내부 링크 기반이 되는 파트부터 채우는 거예요. 파트 7은 위 순서와 무관하게 수시로 챕터가 추가돼요.

## 집필 규칙 요약

- 각 챕터는 하나의 검색 질문에 완결로 답해요 (제목도 검색 질문형으로 지어요).
- 앞 챕터 지식이 필요하면 한두 줄 요약 + 내부 링크로 처리해요.
- 초보 눈높이의 구체적 예시를 챕터당 최소 1개 넣어요.
- 파트 7 글에는 반드시 작성일을 명시하고, 낡아도 삭제하지 않고 아카이브로 유지해요.
- 트렌드 콘텐츠는 집필 시 웹 리서치를 병행해요 (지식 컷오프 이후 정보 반영).
- 파일 형식은 마크다운이고, 도형·다이어그램은 mermaid를 우선 쓰고 복잡한 그림은 각 파트 폴더 하위 `images/`에 저장해요.

전체 설계 배경과 확정 근거는 설계 문서를 참고해 주세요: [docs/superpowers/specs/2026-07-27-stock-wiki-design.md](docs/superpowers/specs/2026-07-27-stock-wiki-design.md)

새 챕터를 쓸 때는 [docs/chapter-template.md](docs/chapter-template.md) 템플릿을 기준으로 작성해요.
