...
- [ ] **Step 2: 이월된 Minor 지적 처리**

집필 단계에서 별도 수정 라운드를 열지 않고 이월한 항목들. 각각 사실 확인 후 반영한다.

1-5 (수수료와 세금):
- 온라인 수수료 "0.01~0.02% 안팎"은 비대면(온라인 전용) 계좌 기준. 영업점 개설 계좌의 온라인 채널은 0.147% 수준으로 10배 차이 → "비대면 전용 계좌 기준" 한정어 추가
- 유관기관 수수료를 표에서는 별도 행으로, 계산 예시에서는 "수수료율에 포함"으로 처리해 표와 예시가 어긋남. 실무상 증권사 위탁수수료에 포함 청구되므로 표 셀에 그 취지 한 줄 추가
- 비상장 0.35%에 K-OTC 예외(0.20%) 누락
- 코넥스가 파트 1 전체에서 이 챕터에만 등장하는데 정의 없음 → "중소·벤처기업용 시장" 정도의 삽입구 추가
- 배당소득세가 표 행으로만 설명되고 정의 문장이 없음

1-6 (시가총액·주가·거래량):
- 발행주식수가 변하는 원인으로 증자·액면분할은 서술했으나 소각(감자)이 빠짐

- [ ] **Step 3: 마크다운·다이어그램 위생 일괄 검사**

```bash
cd /Users/yshyuk/Documents/Repository/stock-wiki
# 볼드 미닫힘 패턴 (CommonMark에서 )**한글 은 볼드를 닫지 못함)
grep -nE '\)\*\*[가-힣]' part1-basics/*.md README.md || echo "볼드 패턴 OK"
# ** 홀수 개 라인
for f in part1-basics/*.md README.md; do
  awk -v F="$f" '{n=gsub(/\*\*/,"&"); if(n%2==1) print F":"NR}' "$f"
done
# frontmatter 6필드
for f in part1-basics/1-*.md; do
...
mermaid 블록이 있는 챕터(1-1, 1-2)는 렌더링 검증이 미실시 상태다. `npx -y @mermaid-js/mermaid-cli` 등으로 실제 파싱을 시도하고, 설치가 무거워 불가하면 라벨 내 괄호·따옴표·슬래시·화살표 부재를 정적 검사로 확인하고 그 사실을 리포트에 남긴다.

- [ ] **Step 4: 톤·중복·정합성 통독 검수**

파트 1 전체(6개)를 1-1부터 순서대로 통독하며 확인한다:
- 말투 통일 (해요체)
- 같은 설명의 과도한 중복
- 챕터 간 난이도 역전
- 용어 정의 충돌 (예: 동시호가/단일가매매, 발행주식수/상장주식수처럼 여러 챕터에 걸친 용어가 서로 다르게 정의되어 있지 않은지)
- 내부 링크가 실제로 독자를 올바른 곳으로 보내는지

발견 시 수정한다.

- [ ] **Step 5: README 파트 1 체크박스 6개가 모두 [x]인지 확인 후 Commit**
