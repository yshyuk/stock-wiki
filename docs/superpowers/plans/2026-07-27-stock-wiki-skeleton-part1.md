...
3. 전체 목차: 설계 문서 5장의 7개 파트 40개 챕터를 체크박스 목록으로. 각 항목은 `- [ ] [1-1. 주식이란 무엇인가 — 회사 지분을 산다는 것의 의미](part1-basics/1-1-what-is-stock.md)` 형식 (체크박스가 곧 진행 현황). 파일명은 아래 슬러그 표를 그대로 사용:
...
- 섹션 1 "회사는 왜 주식을 발행하나": 자금 조달 관점. mermaid flowchart 필수 — `회사 --주식 발행--> 투자자`, `투자자 --투자금--> 회사`, `투자자 --배당/시세차익--> 수익`
...
- 섹션 3 "시간외 거래는 언제 쓰나": 장 중 매매 못 하는 직장인 사례
