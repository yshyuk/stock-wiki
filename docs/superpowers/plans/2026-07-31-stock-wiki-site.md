...
이 환경은 비대화형이라 브라우저를 직접 열 수 없었다. 아래 표는 **자동화로 확인 가능한 범위**까지만
채웠고, 육안 확인이 필요한 항목은 명시적으로 "미확인"으로 남겼다. 자동화로 확인한 것:
...
「렌더」「한글」 두 열은 위 자동화 증거(변환 파이프라인 동작 + sanitize 직전까지 파서 통과)까지만
확인됐고, 실제 그림으로 보이는지·한글 라벨이 깨지지 않는지는 브라우저 육안 확인이 필요하다.
「다크모드」「모바일」은 이 환경에서 확인할 방법이 전혀 없다(둘 다 클라이언트 런타임 동작).

| 파일 | 렌더 | 한글 | 다크모드 | 모바일 | 수정 여부 |
|---|---|---|---|---|---|
| 1-1 | 미확인(브라우저 필요) — 변환·파서 통과까지 확인 | 미확인(브라우저 필요) — 라벨 텍스트까지 파서 도달 확인 | 미확인(브라우저 필요) | 미확인(브라우저 필요) | 아니오 |
| 1-2 | 미확인(브라우저 필요) — 변환·파서 통과까지 확인 | 미확인(브라우저 필요) — 라벨 텍스트까지 파서 도달 확인 | 미확인(브라우저 필요) | 미확인(브라우저 필요) | 아니오 |
| 2-4 | 미확인(브라우저 필요) — 변환·파서 통과까지 확인 | 미확인(브라우저 필요) — 라벨 텍스트까지 파서 도달 확인 | 미확인(브라우저 필요) | 미확인(브라우저 필요) | 아니오 |
| 2-7 | 미확인(브라우저 필요) — 변환·파서 통과까지 확인 | 미확인(브라우저 필요) — 라벨 텍스트까지 파서 도달 확인 | 미확인(브라우저 필요) | 미확인(브라우저 필요) | 아니오 |

**다음에 브라우저가 있는 세션에서 할 일**: `npm run preview` 실행 후 위 네 경로를 열어 표의 네 열을
실제 값(예/아니오, 필요하면 스크린샷)으로 갱신한다. 이 절은 그때까지 "미확인"이 정직한 상태다.
