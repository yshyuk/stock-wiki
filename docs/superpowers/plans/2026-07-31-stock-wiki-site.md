...
이 페이지의 시행일(2026년 8월 4일) 기준으로 이 사이트는 아직 광고를 게재하고 있지 않아요. 아래 내용은 구글 애드센스 심사를 통과해서 광고가 실제로 붙기 시작하면 적용되는 내용이에요.
...
- **mermaid 다이어그램 4건의 문법·렌더 확인 완료 (2026-08-04).** 1-1, 1-2, 2-4, 2-7의 다이어그램
  ...
  **그동안 「정적 문법 검사만 거쳤고 실제 렌더는 아무도 확인하지 못했다」던 상태는 이것으로
  끝났어요.** 다만 확인에 쓴 페이지는 사이트와 다른 mermaid 렌더러를 썼으므로, 사이트 자체에서의
  다크 모드·레이아웃·모바일 표시는 별개로 남아 있어요(아래 「남은 사항」).
...
- **mermaid 다이어그램의 사이트 내 표시 — 배포 후 확인 남음.** 다이어그램 문법 자체는 정상으로
  확인됐어요(위 「해결된 숙제」 참고). 남은 건 **실제 사이트 안에서의 표시**예요. 사이트는
  `vitepress-plugin-mermaid`로 렌더하는데, 확인에 쓴 페이지는 다른 렌더러였어서 다크 모드 전환,
  3단 레이아웃 안에서의 크기·여백, 375px 모바일 폭에서의 가로 스크롤은 그 확인에 포함되지 않아요.
  배포 후 `/part1-basics/1-1-what-is-stock`, `/part1-basics/1-2-open-brokerage-account`,
  `/part2-korea-market/2-4-ipo-subscription`, `/part2-korea-market/2-7-short-selling` 네 페이지를
  열어 확인하세요.
- **사이트 전반의 시각 확인 — 배포 후 남음.** 3단 레이아웃, 다크 모드, 모바일 폭에서의 사이드바
  햄버거 접힘은 비대화형 환경이라 확인하지 못했어요. 빌드 산출물 검사로는 마크업이 정상이라는
  것까지만 확인했어요.
...
이 절은 두 단계로 채워졌다. Task 5 시점에는 비대화형 환경이라 자동화 증거까지만 모았고,
2026-08-04에 사이트 소유자가 브라우저에서 다이어그램 네 건을 직접 확인해 「렌더」·「한글」 열이
확정됐다. 먼저 Task 5에서 자동화로 확인한 것:

1. **변환 파이프라인**: `npm run build` 성공. 네 페이지의 `.vitepress/dist/**/*.html`을 grep한 결과,
   `language-mermaid`(원본 코드 블록) 0건, `class="mermaid"`(플러그인이 생성한 마운트 지점) 1건씩 —
   `withMermaid`가 실제로 적용되어 코드 블록이 그대로 노출되지 않음을 확인했다. 단, 이 div는 SSG
   산출물에서는 비어 있고 클라이언트 JS가 마운트 시점에 그려 넣으므로, 이 grep은 "그림이 실제로
   보인다"까지는 증명하지 못한다.
2. **mermaid 자체 파서 검증**: 네 다이어그램 소스를 추출해 `mermaid.parse()`를 직접 호출했다. 네
   건 모두 동일한 지점, 동일한 오류로 실패했다 — `TypeError: DOMPurify.addHook is not a function`
   (`FlowDB.addVertex` → `sanitizeText` 경로, DOMPurify가 `window`/`document` 없이는 완전히
   동작하지 않음). 4건 모두 같은 실패 지점이라는 것은 다이어그램별 문법 차이가 아니라 Node
   환경에 DOM 전역이 없다는 환경 제약임을 시사한다. jison 파서는 이 지점까지(꺾쇠 토큰, 라벨,
   화살표를 포함한 정점 추가) 문제없이 진행했으므로, sanitize 이전 단계의 문법 자체는 4건 모두
   깨지지 않았다. 브리핑 지시대로 jsdom 등 새 의존성을 추가해 이 오류를 우회하지 않았다.

위 자동화 증거는 「변환 파이프라인이 동작한다」와 「sanitize 직전까지 문법이 깨지지 않았다」까지를
보였다. 실제로 그림이 되는지는 아래 브라우저 확인으로 채워졌다.

| 파일 | 렌더 | 한글 | 다크모드 | 모바일 | 수정 여부 |
|---|---|---|---|---|---|
| 1-1 | 정상 | 정상 | 미확인(사이트 배포 후) | 미확인(사이트 배포 후) | 없음 |
| 1-2 | 정상 | 정상 | 미확인(사이트 배포 후) | 미확인(사이트 배포 후) | 없음 |
| 2-4 | 정상 | 정상 | 미확인(사이트 배포 후) | 미확인(사이트 배포 후) | 없음 |
| 2-7 | 정상 | 정상 | 미확인(사이트 배포 후) | 미확인(사이트 배포 후) | 없음 |

**렌더·한글 열의 근거 (2026-08-04)**: 네 챕터의 mermaid 블록을 한 글자도 바꾸지 않고 옮긴 확인용
페이지를 만들어 사이트 소유자가 브라우저에서 직접 확인했다. 네 개 모두 그림으로 렌더되고 한글
라벨이 깨지지 않았으며, 1-1의 화살표 라벨 네 개도 제자리에 붙었다. 다이어그램 문법 수정은 필요
없었다 — 제1 제약의 mermaid 예외는 사용되지 않았다.

**다크모드·모바일 열이 미확인인 이유**: 확인용 페이지는 사이트와 다른 mermaid 렌더러를 쓴다.
사이트는 `vitepress-plugin-mermaid`가 body의 `dark` 클래스를 감지해 테마를 바꾸고, 다이어그램은
3단 레이아웃의 본문 열 안에 놓인다. 두 조건 모두 확인용 페이지에는 없었으므로 배포 후 실제
사이트에서 확인해야 한다.

**남은 일**: 배포 후 실제 사이트에서 네 페이지를 열어 「다크모드」·「모바일」 두 열을 채운다.
다이어그램 문법은 확정됐으므로, 여기서 문제가 나온다면 그것은 다이어그램이 아니라
`vitepress-plugin-mermaid`의 테마 연동이나 3단 레이아웃의 폭 처리 쪽이다.
...
### 위 수치가 재는 것: 모형이지 실제 색인이 아니다

`tests/helpers/search-index.js`의 `buildSearchModel`은 VitePress가 실제로 만드는 색인이 아니라, **같은 필드 구성(`title`/`titles`/`text`)과 같은 검색 옵션**으로 실제 챕터 본문을 색인한 모형이다. `@localSearchIndex` 청크를 직접 파싱하지 않은 이유는, 그건 VitePress 내부 포맷이라 업그레이드 한 번에 깨질 수 있는 결합을 만들기 때문이다. 이 모형과 실제 색인 사이에는 브리프 Step 6 코드가 그대로 지정한 두 가지 구조적 차이가 있다.

1. **문서 단위** — 모형은 파일 1개 = 문서 1개(13건)로 다루지만, VitePress는 페이지를 heading 섹션 단위로 쪼개 문서를 만든다. 실제 색인은 13개 파일에서 **98개 섹션 문서**가 나온다.
2. **keywords가 들어가는 필드** — 모형은 `keywords`를 `titles` 필드(가중치 1)에 넣지만, 실제 시스템에서 `_render`가 주입한 keywords는 h1 섹션의 **`text` 필드**(가중치 2)에 들어간다.

그래서 회귀 테스트의 통과/실패와 측정 스크립트의 정확한 수치는 모형이라는 구조가 다른 대상에 대한 결과이지, 배포되는 색인 자체를 증명하지는 않는다.

**Task 4 리뷰 중 실제 색인으로 교차검증한 결과** (`.vitepress/dist/assets/chunks/@localSearchIndexroot.Bg0M_sJV.js`를 `MiniSearch.loadJSON`으로 로드):

- `"공매도 재개"` — 본문에는 없고 keywords로만 주입된 문구. 최상위 결과가 `/part2-korea-market/2-7-short-selling#공매도란-무엇이고-왜-논란인가`(제목 "공매도란 무엇이고 왜 논란인가"), 점수 **103.96** (2위는 55.67). keywords 주입이 실제 색인에서도 동작하고, 챕터 자신의 h1 섹션에 붙지 유령 빈 제목 섹션에 붙지 않음을 확인.
- `거래소` — 실제 색인 **49건** 히트, 한국거래소를 다루는 2-1(코스피 코스닥 차이)이 **20/49위**. 모형이 보여준 "13건 중 13위" 손실과 질적으로 같은 실패.
- `증자` — 실제 색인에서 2-5(유상증자·무상증자·액면분할)가 **1위**.
- `grep -o "search-keywords" .vitepress/dist/part2-korea-market/2-7-short-selling.html` → 매치 없음. `_render`는 색인에만 반영되고 렌더링된 페이지 본문에는 나타나지 않음을 확인.

**결론**: 구조가 다른 모형과 실제 색인 양쪽에서 같은 그림이 나온다 — keywords 주입은 동작하고, 조사 부착 질의는 잡히며, 합성어 내부 일치(`거래소`)만 새고 그 손실은 국지적이다. "접미사 색인은 지금 도입하지 않는다"는 판단은 모형에서도 실제 색인에서도 유효하다.
